...
function isWorkboxDisabled (nuxt) {
  const { pwa } = nuxt.options
  const workboxEnabled = pwa && pwa.workbox && pwa.workbox.enabled
  const isDev = nuxt.options.dev

  return (isDev && workboxEnabled === undefined) || workboxEnabled === false
}

export default function nuxtOneSignal (oneSignalOptions) {
  // If workbox is disabled it registers a self-destructing worker
  // If this worker is added as an import to oneSignal worker
  // that results into an infinitie page reloading cycle
  // There's also no point in running oneSignal without workbox
  if (isWorkboxDisabled(this)) {
    return
  }
